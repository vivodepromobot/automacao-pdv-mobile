...
def run_test_venda_cliente():
    ...
    try:
        ...
        executar_passo("Clicar no botão 'Iniciar Venda'", lambda: clicar_por_texto(driver, "Iniciar Venda"))
        executar_passo("Selecionar Vendedor", lambda: clicar_por_id(driver, "txt_dialog_seller_name"))

        executar_passo("Clicar no botão 'Buscar Cliente'",
                   lambda: clicar_por_id(driver, "btn_select_customer"))
        
        # Usando a função para selecionar o cliente
        ...
        executar_passo("Responder SIM para Imprimir Comprovante (aguardando até 20s)", 
                       lambda: clicar_em_id_se_existir(driver, "android:id/button1", wait_time=20))
        ...
        logger.error(f"ERRO CRÍTICO DURANTE A EXECUÇÃO DO TESTE DE VENDA CLIENTE: {e}")
        if driver: save_screenshot(driver, "CRITICO_VENDA_CLIENTE")
    finally:
        end_time = datetime.now()
        status = "SUCESSO_VENDA_CLIENTE" if teste_passou else "FALHA_VENDA_CLIENTE"
